...
// include file parsers
require dirname( __FILE__ ) . '/parsers.php';

if ( !class_exists( 'Noo_Import_Demo' ) ) :

	class Noo_Import_Demo extends WP_Importer{
		var $max_noo_version   = 1.2;
		...
		public function process_data() {
			...
			// --- load file demo
				...
				$file_demo = dirname( __FILE__ ) . "/data-demo/{$_POST['name']}/content.xml";
				$widget = dirname( __FILE__ ) . "/data-demo/{$_POST['name']}/widgets.wie";
				$option = dirname( __FILE__ ) . "/data-demo/{$_POST['name']}/option.json";


				$this->import( $file_demo );
				$this->noo_process_import_file( $widget );
				$this->process_option( $option );
				echo esc_html__( 'Import Successfully!', 'noo' );
		...
		function import_end() {
			...
			echo esc_html__( 'All done. Have fun!', 'noo' );
		...
		function author_select( $n, $author ) {
			echo esc_html__( 'Import author:', 'noo' );
			...
			if ( $create_users ) {
				if ( $this->version != '1.0' ) {
					echo esc_html__( 'or create new user with login name:', 'noo' );
					$value = '';
				} else {
					echo esc_html__( 'as a new user:', 'noo' );
			...
			if ( ! $create_users && $this->version == '1.0' )
				echo esc_html__( 'assign posts to an existing user:', 'noo' );
			else
				echo esc_html__( 'or assign posts to an existing user:', 'noo' );
			...
			foreach ( (array) $_POST['imported_authors'] as $i => $old_login ) {
				...
				if ( ! empty( $_POST['user_map'][$i] ) ) {
					...
					if ( ! is_wp_error( $user_id ) ) {
						...
						if ( defined('NOO_IMPORT_DEBUG') && NOO_IMPORT_DEBUG ) {
				...
				*/
				...
				if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
					...
					if ( is_wp_error( $post_id ) ) {
						if ( defined('NOO_IMPORT_DEBUG') && NOO_IMPORT_DEBUG ) {
							printf( esc_html__( 'Failed to import %s &#8220;%s&#8221;', 'noo' ),
		...
		function process_menu_item( $item ) {
			...
			if ( ! $menu_slug ) {
				echo esc_html__( 'Menu item skipped due to missing menu slug', 'noo' );
				echo '<br />';
				return;
			}
		...
		function noo_available_widgets() {
			...
			return apply_filters( 'noo_available_widgets', $available_widgets );
		 ...
		 *
		 * @since 0.3
		 * @param string $file Path to .noo file uploaded
		 * @global string $noo_import_results
		 */
		function noo_process_import_file( $file ) {

			global $noo_import_results;

			// File exists?
			// if ( ! file_exists( $file ) ) {
			// 	wp_die(
			// 		esc_html__( 'Import file could not be found. Please try again.', 'noo' ),
			// 		'',
			...
			$noo_import_results = $this->noo_import_data( $data );
		 ...
		 * @param object $data JSON widget data from .noo file
		 * @return array Results array
		 */
		function noo_import_data( $data ) {
			...
			// Hook before import
			do_action( 'noo_before_import' );
			$data = apply_filters( 'noo_import_data', $data );

			// Get all available widgets site supports
			$available_widgets = $this->noo_available_widgets();
			...
			foreach ( $data as $sidebar_id => $widgets ) {
				...
				foreach ( $widgets as $widget_instance_id => $widget ) {
					...
					// Ideally the newer noo_widget_settings_array below will be used instead of this
					$widget = apply_filters( 'noo_widget_settings', $widget ); // object
					...
					// This is preferred over the older noo_widget_settings filter above
					// Do before identical check because changes may make it identical to end result (such as URL replacements)
					$widget = apply_filters( 'noo_widget_settings_array', $widget );
					...
					if ( ! $fail ) {
						...
						do_action( 'noo_after_widget_import', $after_widget_import );
			...
			// Hook after import
			do_action( 'noo_after_import' );

			// Return results
			return apply_filters( 'noo_import_results', $results );
		...
		function process_option( $file ) {
			...
			foreach ( (array) $options_to_import as $option_name ) {
				if ( isset( $data['options'][ $option_name ] ) ) {
					...
					// remove primary menu
					$index_option = get_option( $option_name, true );
					if ( in_array( 'nav_menu_locations', $index_option ) ) :

						unset( $index_option['nav_menu_locations'] ); // remove
						update_option( $option_name, $index_option );
				...
				if( $left_menu = wp_get_nav_menu_object( 'Main Menu Left' ) ) {
					$nav_menu_locations['left-menu'] = $left_menu->term_id;
				}
				if( $primary_menu = wp_get_nav_menu_object( 'Main Menu' ) ) {
					$nav_menu_locations['primary'] = $primary_menu->term_id;
				}
				if( $right_menu = wp_get_nav_menu_object( 'Main Menu Right' ) ) {
					$nav_menu_locations['right-menu'] = $right_menu->term_id;
				}
		...
		private function get_whitelist_options() {
			...
			return apply_filters( 'options_import_whitelist', array(
				...
				'page_on_front',
				'show_on_front',
				'theme_mods_' .  $noo_theme->get( 'TextDomain' ),
				'theme_mods_' .  $noo_theme->get( 'TextDomain' ) . '-child'
			) );
		}
