...
class Noo_Notice_Install {
	...
	private $product_id = 'noo-organici';
	...
	public function notice_html_install() {
		...
		<div id="message" class="updated notice is-dismissible">
			<p><strong><?php echo sprintf( esc_html__( 'Welcome to %s,', 'noo'), wp_get_theme($this->product_id)->get('Name') ); ?></strong></p>
			<p><?php echo esc_html__( 'If it is the first time you install this theme, you should go and check the basic setting.', 'noo'); ?></p>
			<p class="submit">
				<a href="<?php echo esc_url( admin_url( 'admin.php?page=noo-setup' ) ); ?>" class="button-primary">
					<?php echo esc_html__( 'Go to Quick Setup', 'noo' ); ?>
				</a>
				<a href="<?php echo esc_url( admin_url( 'admin.php?page=noo-setup&action=skip' ) ); ?>" class="button">
					<?php echo esc_html__( 'Skip Setup', 'noo' ); ?>
				</a>
			</p>
		</div>
		<?php
		...
		public function noo_setup() {
			...
			// -----
				...
				if( !is_wp_error( $id_page ) ) {
					...
					echo json_encode( array( 'id' => esc_html__( 'Done', 'noo'), 'slug' => $post->post_name ) );
				}
				exit;
		}
	...
	// -- Tab menu
		public function noo_tab_menu( $current = 'general' ) {
			...
			$tabs = array(
		        'general'   => esc_html__( 'Quick Setup', 'noo' ), 
		        'import_demo'  => esc_html__( 'Import Demo', 'noo' )
		    );
	...
	// -- OPTION
                    ...
                    <table class="widefat" cellspacing="0" id="client">
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <span><?php echo sprintf( wp_kses( __( '<a target="_blank" href="%s">How to get License key?</a>', 'noo-ivent' ), noo_organici_allowed_html() ), 'http://support.nootheme.com/wp-content/uploads/2015/07/HowToGetPurchaseCode.png' ) ?></span>
                            </td>
                        </tr>
                        </tbody>
                    </table>
			...
			public function noo_option_page() {
				...
				?>
					<table class="widefat" cellspacing="0">
						<thead>
							<tr>
								<th colspan="5" data-export-label="<?php echo esc_html__( 'Required Pages', 'noo' ); ?>"><?php echo esc_html__( 'Required Pages', 'noo' ); ?></th>
							</tr>
						</thead>
						<tbody>
							<?php
								...
								$list_pages = array(
									array(
										'title'         => esc_html__( 'Front Page' , 'noo'),
										'content'       => '',
										'page_template' => '',
										'help'          => esc_html__( 'The Homepage', 'noo'),
										'setting'       => array( 
																'group' => '',
																'key'   => 'show_on_front',
																'value' => 'page',
									...
									array(
										'title'         => esc_html__( 'Member' , 'noo'),
										'content'       => '[noo_member_account]',
										'page_template' => '',
										'help'          => esc_html__( 'The main page for all the action of Employer and Candidate', 'noo'),
										'setting'       => array( 
																'group' => 'noo_member',
																'key'   => 'manage_page_id',
									...
									array(
										'title'         => esc_html__( 'Packages' , 'noo'),
										'content'       => '[noo_job_package_list]',
										'page_template' => '',
										'help'          => esc_html__( 'The page for Employer to choose Job Package', 'noo'),
										'setting'       => array( 
																'group' => 'job_package',
																'key'   => 'package_page_id',
									...
									array(
										'title'         => esc_html__( 'Post Job' , 'noo'),
										'content'       => '',
										'page_template' => 'page-post-job.php',
										'help'          => esc_html__( 'The page for Job posting', 'noo'),
										'setting'       => array(),
									),
									array(
										'title'         => esc_html__( 'Post Resume' , 'noo'),
										'content'       => '',
										'page_template' => 'page-post-resume.php',
										'help'          => esc_html__( 'The page for Resume posting', 'noo'),
										'setting'       => array(),
									),
								);
								...
								foreach ($list_pages as $list_page => $page) :
									...
									if( isset( $setting ) && !empty( $setting ) && is_array($setting) && !empty( $setting['key'] ) ) :
										...
										if( $setting_value == null ) :
											...
											if( isset( $setting['value'] ) && $setting['value'] == $setting_value ) {
												...
												if( !empty( $get_page ) && $get_page->post_type == 'page' ) {
													...
													if( ( isset( $page['page_template'] ) && !empty( $page['page_template'] ) && $page['page_template'] == noo_organici_get_post_meta( $get_id, '_wp_page_template' ) ) 
										...
										if( $setting_result == 'missing' ) {
											echo "	<mark class='error'>" . esc_html__( 'Missing setting', 'noo' ) . "</mark>";
										} elseif( $setting_result == 'false' ) {
											echo "<mark class='error'>" . esc_html__( 'Wrong', 'noo' );
											if( !empty( $page['content'] ) ) {
												echo ' - ' . sprintf( esc_html__( 'Page should contains %s', 'noo' ), $page['content'] );
											} elseif ( !empty( $page['page_template'] ) ) {
												echo ' - ' . sprintf( esc_html__( 'Page should have template %s', 'noo' ), $page['page_template'] );
											}
											echo "</mark>";
										} else {
											echo "<mark class='yes'>" . esc_html__( 'Done', 'noo');
										...
										if( $setting_result !== 'true' && ( !empty( $page['content'] ) || !empty( $page['page_template'] ) ) ) {
											...
											echo '		<span class="correct-setting" data-title="' . $page['title'] . '" data-content="' . $page['content'] . '" data-page-template="' . $page['page_template'] . '" data-setting-group="' . $setting['group'] . '" data-setting-key="' . $setting['key'] . '">' . esc_html__( 'Correct now', 'noo') . '</span>';
											echo '	</div>';
										}
										echo '</td>';
										echo '<td>';
										if( $setting_result != 'true' && isset( $setting['url']) && !empty( $setting['url'] ) ) {
											...
											echo "		<a href='{$setting['url']}' title='{$page['title']}' target='_blank'>" . esc_html__('Edit setting', 'noo') . "</a>";
											echo '	</div>';
										}
										echo '</td>';
										...
										if ( !empty($page['page_template']) ) :
											...
											if( !empty( $get_id ) && !empty( $get_page ) ) {
												...
												echo '<td>';
												echo "	<mark class='yes'>" . esc_html__( 'Done', 'noo') . " - /{$get_page->post_name}/</mark>";
												echo '</td>';
											} else {
												echo "<td data-export-label='{$page['title']}'>{$page['title']}</td>";
												if( isset( $page['help'] ) && !empty( $page['help'] ) ) {
												...
												echo '<td>';
												echo "	<mark class='error'>" . sprintf( esc_html__( 'You need to create a page with template %s', 'noo' ), $page['page_template'] ) . "</mark>";
												echo '</td>';
												echo '<td>';
												echo '	<div class="button button-primary">';
												echo '		<span class="correct-setting" data-title="' . $page['title'] . '" data-content="' . $page['content'] . '" data-page-template="' . $page['page_template'] . '">' . esc_html__( 'Correct now', 'noo') . '</span>';
												echo '	</div>';
												echo '</td>';
												echo '<td></td>';
											}
		...
		public function noo_import_demo_options() {
			...
			$list_demo = array(
				array(
	                'name' => esc_html__( 'Organici', 'noo' ),
	                'img'  => NOO_PLUGIN_PATH . 'admin/importer/data-demo/organici/screenshot.jpg',
	                'file' => 'organici'
	            ),
                array(
	                'name' => esc_html__( 'Bakery', 'noo' ),
	                'img'  => NOO_PLUGIN_PATH . 'admin/importer/data-demo/bakery/screenshot.jpg',
	                'file' => 'bakery'
	            )
			);
			...
			?>
				<table class="widefat" cellspacing="0" style="width: 99%;">
					<thead>
						<tr>
							<th colspan="1" data-export-label="<?php echo esc_html__( 'Settings', 'noo' ); ?>">
								<label class="hide_main">
									<?php echo esc_html__( 'Settings', 'noo' ); ?>
								</label>
							</th>
						</tr>
					</thead>
					<tbody id="noo_main_select">
						<tr>
							<td>
								<input type='checkbox' data-id='import_post' id='import_post' value='1' checked /> <?php echo esc_html__( 'Import Post', 'noo' ); ?>
							</td>
						</tr>
						<tr>
							<td>
								<input type='checkbox' data-id='import_nav' id='import_nav' value='1' checked /> <?php echo esc_html__( 'Import Nav Menu', 'noo' ); ?>
							</td>
						</tr>
						<tr>
							<td>
								<input type='checkbox' data-id='import_comment' id='import_comment' value='1' checked /> <?php echo esc_html__( 'Import Comment', 'noo' ); ?>
							</td>
						</tr>
					</tbody>
				</table>
    ...
    public function noo_options_fields() {
        ...
        $this->option_metabox[] = array(
            ...
            'id' => 'general_options',
            'title' => esc_html__( 'General Options', 'noo' )
