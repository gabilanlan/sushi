...
if ( ! function_exists( 'noo_organici_enqueue_site_scripts' ) ) :
	function noo_organici_enqueue_site_scripts() {
		...
		wp_register_script( 'jquery-scroll', get_template_directory_uri() . '/assets/vendor/scrollbar/js/jquery.scrollbar.min.js', null, null, false );

		wp_register_script( 'prettyPhoto', get_template_directory_uri() . '/assets/vendor/prettyPhoto/js/jquery.prettyPhoto.js', null, null, false );
		...
		wp_register_script( 'google-map','http'.(is_ssl() ? 's':'').'://maps.googleapis.com/maps/api/js?v=3.exp&sensor=true&libraries=places' . ( !empty( $google_api ) ? '&key=' .$google_api : '' ), array('jquery'), null , true);
		...
		if ( ! is_admin() ) {
			...
			wp_enqueue_script( 'noo-slidebars', get_template_directory_uri() . '/assets/js/min/slideout.min.js', array(), null, true );
			wp_enqueue_script( 'noo-canvas', get_template_directory_uri() . '/assets/js/off-cavnass.js', array(), null, true );
            $menu_reponsive    = noo_organici_get_option( 'noo_header_menu_responsive', 'default' );
            wp_localize_script('noo-canvas', 'nooCV', array(
            	'eff' => $menu_reponsive
            ));
...
function noo_load_vc_iframe_js() {
...
add_action( 'vc_load_iframe_jscss', 'noo_load_vc_iframe_js' );
add_action( 'admin_enqueue_scripts', 'noo_load_vc_iframe_js' );