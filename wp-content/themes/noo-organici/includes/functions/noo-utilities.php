...
if( !function_exists('noo_organici_new_heading') ){
    function noo_organici_new_heading(){
        ...
        if( NOO_WOOCOMMERCE_EXIST && ( is_shop()  || is_product_category() || is_product_tag() )) {
            ...
            $heading_image = noo_organici_get_post_meta(get_the_ID(),'_heading_image');
            
            if (isset($heading_image) && !empty($heading_image)) {
                $image = wp_get_attachment_url($heading_image);
            }
            $title = get_the_title();
            ...
            $image = noo_organici_get_image_option('noo_blog_heading_image', '');
            $title = noo_organici_get_option('noo_blog_heading_title', single_cat_title('', false));
            ...
            $image_blog = noo_organici_get_image_option('noo_blog_heading_image', '');
            if (isset($image_blog) && !empty($image_blog)) {
                $image = $image_blog;
            }
            if (isset($heading_image) && !empty($heading_image)) {
                $image = wp_get_attachment_url($heading_image);
