...
if ( ! function_exists( 'noo_organici_customizer_register_options_general' ) ) :
	function noo_organici_customizer_register_options_general( $wp_customize ) {
		...
		$helper->add_sub_section(
			'noo_customizer_section_site_enhancement_map',
			esc_html__( 'Google Maps API Key', 'noo-organici' ),
			noo_organici_kses( __( '<strong>Google</strong> requires that you register an API Key to display Maps on from your website. To know how to create this application, <a target="_blank" href="https://support.nootheme.com/knowledge-base/get-google-maps-api/">How to get Google API Key</a> ', 'noo-organici' ) )
		);

			$helper->add_control(
				'noo_google_map_api_key',
				'text',
				esc_html__('Google Maps API Key', 'noo-organici')
			);
	}
...
if ( ! function_exists( 'noo_organici_customizer_register_options_layout' ) ) :
	function noo_organici_customizer_register_options_layout( $wp_customize ) {
        ...
        $helper->add_control(
            ...
            'alpha_color',
            esc_html__( 'Secondary Color', 'noo-organici' ),
            noo_organici_get_theme_default( 'secondary_color' ),
            array(
                'preview_type'   => 'update_css',
                'preview_params' => array( 'css' => 'design' ),
                'show_opacity'   => true
...
if ( ! function_exists( 'noo_organici_customizer_register_options_header' ) ) :
	function noo_organici_customizer_register_options_header( $wp_customize ) {
        ...
        // Sub-Section: Responsive Menu
        $helper->add_sub_section(
            'noo_header_sub_section_responsive',
            esc_html__( 'Mobile Menu', 'noo-organici' ),
            esc_html__( 'Choose the style of your menu on mobile devices. Slide from right to left or standard drop-down top to bottom.', 'noo-organici' )
        );

      	// Control: Responsive Menu
        $helper->add_control(
            'noo_header_menu_responsive',
            'noo_radio',
            esc_html__( 'Mobile Menu Style', 'noo-organici' ),
            'default',
            array(
                'choices' => array(
					'default' => esc_html__( 'Right to Left', 'noo-organici' ),
					'top'     => esc_html__( 'Top to Bottom', 'noo-organici' ),
                )
            )
        );
		...
		if( NOO_WOOCOMMERCE_EXIST ) {
			...
			$helper->add_control(
				...
				array(
					'json' => array(
						'on_child_options' => 'noo_header_nav_icon_cart_on_mobile'
					)
				)
			);
			$helper->add_control(
				'noo_header_nav_icon_cart_on_mobile',
				'noo_switch',
				esc_html__( 'Show Shopping Cart on mobile', 'noo-organici' ),
				0
			);
		}
        ...
        $helper->add_control(
        	'noo_header_top_on',
        	'noo_switch',
			esc_html__( 'Show Topbar', 'noo-organici' ),
        	0,
        	array(
        		'json' => array(
        			'on_child_options' => 'noo_header_top_phone, noo_header_top_email,
        								noo_header_nav_top_icon_cart, noo_header_nav_top_icon_search,
        								noo_header_nav_top_icon_user, noo_header_nav_top_icon_wishlist'
    			)
    		)
    	);
		...
		// Control: Show Search
		$helper->add_control(
			'noo_header_nav_top_icon_search',
			'noo_switch',
			esc_html__( 'Show Search Icon', 'noo-organici' ),
			0,
			array(
		...
		// Control: Show Wishlist
		$helper->add_control(
			'noo_header_nav_top_icon_wishlist',
			'noo_switch',
			esc_html__( 'Show Wishlist Icon', 'noo-organici' ),
			0,
			array(
		...
		// Control: Show social
		$helper->add_control(
			'noo_header_nav_icon_social',
			'noo_switch',
			esc_html__( 'Show social Icon', 'noo-organici' ),
			0,
			array(
				'json' => array(
					'on_child_options'  => 'noo_header_nav_widget'
				)
			)
		);
        ...
        // Control: Navbar Top Meta Social Network
        $helper->add_control(
            'noo_header_nav_widget',
            'select',
            esc_html__('Select widget', 'noo-organici'),
            'select_widget',
            array(
                'choices' => $arr_widget
            )
...
if ( ! function_exists( 'noo_organici_customizer_register_options_blog' ) ) :
	function noo_organici_customizer_register_options_blog( $wp_customize ) {
		...
		$helper->add_control(
			'noo_blog_style',
			'noo_radio',
			esc_html__( 'Blog Style', 'noo-organici' ),
			'list',
			array(
				'choices' => array(
					'list'   => esc_html__( 'List', 'noo-organici' ),
					'masonry'   => esc_html__( 'Masonry', 'noo-organici' )
				),
				'json' => array(
					'child_options' => array(
		...
		$helper->add_control(
			...
			'select',
			esc_html__( 'Masonry Columns', 'noo-organici' ),
			'3',
			array(
				'choices' => array(
					'2'         =>  esc_html__('2 columns','noo-organici'),
	                '3'         =>  esc_html__('3 columns','noo-organici'),
	                '4'         =>  esc_html__('4 columns','noo-organici')
				),
			)
		);
 		...
 		function noo_organici_customizer_register_options_woocommerce( $wp_customize ) {
 			...
 			$helper->add_control(
 				...
 				array(
 					...
 					'json' => array(
 						'child_options' => array(
 							'grid'   => 'noo_shop_display_style'
 						)
 					)
 				)
 			);
 			$helper->add_control(
				'noo_shop_display_style',
				'select',
				esc_html__( 'Shop Display Style ', 'noo-organici' ),
				'fitRows',
				array(
					'choices' => array(
						'fitRows'     => esc_html__( 'FitRows', 'noo-organici' ),
						'masonry'     => esc_html__( 'Masonry', 'noo-organici' )
					)
				)
			);
