	...
	function noo_organici_page_meta_boxes() {
		...
		if( noo_organici_get_option('noo_page_heading', true) ) {
			$meta_box['fields'][] = array(
				'label' => esc_html__( 'Hidden Heading Image.', 'noo-organici'),
				'id' => "_hidden_heading_image",
				'type' => 'checkbox',
				'child-fields' => array(
					'off' => "_heading_image"
				),
		...
		$meta_box = array(
			...
			'title' => esc_html__( 'Sidebar', 'noo-organici'),
			'context'      => 'side',
			'priority'     => 'default',
			'fields' => array(
				array(
					'label' => esc_html__( 'Page Sidebar', 'noo-organici') ,
					'id' => "{$prefix}_sidebar",
					'type' => 'sidebars',
					'std' => 'sidebar-main'
				) ,
