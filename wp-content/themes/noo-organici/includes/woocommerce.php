...
// Remove each style one by one
	...
	function noo_organici_add_to_cart_fragments( $fragments ) {
		...
		$output_cart_header5 = noo_organici_minicart( false, false, esc_html__( 'MY CART', 'noo-organici' ) );
		$fragments['.cart_header5'] = $output_cart_header5;
		return $fragments;
	}
	add_filter( 'woocommerce_add_to_cart_fragments', 'noo_organici_add_to_cart_fragments' );
	...
	function noo_organici_minicart_mobile() {
		...
		$cart_count = $woocommerce->cart->cart_contents_count;
		$cart_output .= '<a class="mobile-minicart-icon" href="' . esc_url(wc_get_cart_url()) . '" title="' . esc_html__( 'View Cart', 'noo-organici' ) .
			 '"><span class="has-cart"><i class="fa fa-shopping-cart"></i><em>' . esc_html($cart_count) . '</em></span></a>';
		return $cart_output;
	}
	
	// Menu cart
	function noo_organici_minicart( $content = false, $show_price = false, $show_text = '' ) {
		global $woocommerce;
		...
		if ( !empty( $show_text ) ) {
			...
			// $output .= '<li id="nav-menu-item-cart" class="menu-item noo-menu-item-cart minicart">';
			$output .= '<li class="cart_header5">';
            $output .= '    <div>';
            $output .= '        <a href="' . esc_url(wc_get_cart_url()) . '">';
            $output .= '        <span class="has-cart ' . esc_html( $cart_has_items ) . '"">';
            $output .= '            <i class="fa fa-shopping-cart"></i>';
            $output .= '            <em>' . esc_html( $cart_count ) . '</em>';
            $output .= '        </span>';
            $output .= '        </a>';
            $output .= '    </div>';
            $output .= '    <div>';
            $output .= '        <a href="' . esc_url(wc_get_cart_url()) . '"><span>' . esc_html( $show_text ) . '</span></a>';
            $output .= '        <span class="price">' . $cart_total . '</span>';
            $output .= '    </div>';
            $output .= '</li>';
		...
		if ( $show_price ) {
			...
			$output .= '<li id="nav-menu-item-cart" class="menu-item noo-menu-item-cart minicart">';
			$output .= '	<a title="' . esc_html__( 'View cart', 'noo-organici' ) . '" href="' . esc_url(wc_get_cart_url()) . '">';
			$output .= '		<span class="has-cart ' . esc_html( $cart_has_items ) . '">';
			$output .= '			<i class="fa fa-shopping-cart"></i>';
			$output .= '			<em>' . esc_html( $cart_count ) . '</em>';
			$output .= '		</span>';
			$output .= '	</a>';
			$output .= '	&nbsp; ‑ &nbsp; <span class="amount">' . $cart_total . '</span>';
			$output .= '</li>';
		...
		if ( ! $content ) {
			$output .= '<li id="nav-menu-item-cart" class="menu-item noo-menu-item-cart minicart"><a title="' .
				 esc_html__( 'View cart', 'noo-organici' ) . '" class="cart-button" href="' . esc_url(wc_get_cart_url()) .
				 '">' . '<span class="cart-item' . esc_html($cart_has_items) . '"><i class="fa fa-shopping-cart"></i>';
			if ( $cart_count != "0" ) {
				$output .= "<span>(" . esc_html($cart_count) . ")</span> " . esc_html__('Items', 'noo-organici');
			} else {
				$output .= esc_html__('Cart', 'noo-organici');
			}
			$output .= '</span>';
			$output .= '</a>';
		...
		if ( $cart_count != "0" ) {
			...
			$output .= '<div class="minicart-body scrollbar-inner">';
			foreach ( $woocommerce->cart->cart_contents as $cart_item_key => $cart_item ) {
				...
				if ( $cart_product->exists() && $cart_item['quantity'] > 0 ) {
					...
					$output .= '<div class="cart-product-title"><a href="' . get_permalink( $cart_item['product_id'] ) .
						 ...
						 '</a></div>';
					$output .= '<div class="cart-product-price">' . wc_price( $cart_product->get_price() ) . '</div>';
	...
	function noo_organici_woocommerce_shop_columns() {
		...
		$col = noo_organici_get_option('noo_shop_grid_column',4);
		if (empty($col)){
			$col = (noo_organici_get_option( 'noo_shop_layout', 'fullwidth' ) === 'fullwidth') ? 4 : 3;
		}
	...
	function noo_organici_template_loop_quickview() {
		global $product;
		echo '<a class="shop-loop-quickview" data-product_id ="' . esc_attr($product->get_id()) . '" href="' . esc_url($product->get_permalink()) .
	...
	function noo_organici_woocommerce_output_related_products_args() {
		...
		if ( noo_organici_get_option( 'noo_shop_layout', 'fullwidth' ) === 'fullwidth' ) {
			...
			$args = array( 'posts_per_page' => 4, 'columns' => $col );
			return $args;
		}
		$args = array( 'posts_per_page' => noo_organici_get_option('noo_woocommerce_product_related',6), 'columns' => $col );
		return $args;
	}
    ...
    function noo_organici_template_loop_product_thumbnail() {
        ?>
            <div class="noo-product-thumbnail">
                <?php
                    the_post_thumbnail( array(240, 300) );
    ...
    function noo_organici_boxed_tabs_content(){
        ...
        if( isset($box_contents) && !empty($box_contents) ):
            echo '<ul>';
                ...
                <?php }
            echo '</ul>';
            echo '<p class="ds">'.esc_html__('We’ll do our best to send you the things you’ve chosen. Our photos don’t show the actual quantities.','noo-organici').'</p>';
        endif;
    }
    ...
    function noo_organici_product_cat($order, $orderby, $posts_per_page, $new_cat){
        $args = array(
            'post_type'         =>  'product',
            'orderby'           =>   $orderby,
            'order'             =>   $order,
