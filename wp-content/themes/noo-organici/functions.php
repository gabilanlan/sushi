...
function noo_organici_search_form() {
    $form = '<form method="get" class="form-horizontal" action="' . home_url( '/' ) . '" >
        <label class="note-search">'.esc_html__( 'Type and Press Enter to Search', 'noo-organici' ).'</label>
        <input type="search" name="s" class="form-control" value="' . get_search_query() . '" placeholder="'.esc_html__( 'Search...', 'noo-organici' ).'">
...
function noo_organici_custom_menu_item($items, $args){
    if ($args->theme_location == 'right-menu') {
        ...
        if(defined('WOOCOMMERCE_VERSION') && noo_organici_get_option('noo_header_nav_icon_cart',false)):
            $items .= '<li class="menu-item menu-cart-item">';
            $items .= noo_organici_minicart_mobile();
            $items .= '</li>';
        endif;
    }
    return $items;
